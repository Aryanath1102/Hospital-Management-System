...
const rejectLeave = async (req) => {
    ...
    switch (leave.LeaveStatus) {
        case "Rejected":
            return req.error(400, "Leave is already rejected.");
            break;
        case "Approved":
            return req.error(400, "Approved leave cannot be rejected.");
            break;
        case "Cancelled":
            return req.error(400, "Cancelled leave cannot be rejected.");
            break;

        default: break;
    }

    await tx.run(UPDATE(DoctorLeaves).set({LeaveStatus:"Rejected"}).where({ID:leaveID}));
   
    return req.notify("Leave rejected successfully.");
